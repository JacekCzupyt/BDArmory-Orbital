...
public class BallisticsUtils {
    ...
    public float BallisticTrajectorySimulation(
        ref Vector3 position,
        Vector3 velocity,
        float maxTime,
        float timeStep,
        bool ignoreWater = false,
        SimulationStage stage = SimulationStage.Normal
    ) {
        ...
        Ray ray = new Ray();
        if (drawLines) DrawLines(position);
        ...
        while (elapsedTime < maxTime) {
            ...
            if (stage == SimulationStage.Normal && elapsedTime + timeStep > maxTime) {
                // Out of time
                ...
                BallisticTrajectorySimulation(
                    ref position,
                    velocity,
                    maxTime - elapsedTime,
                    (maxTime - elapsedTime) / 4f,
                    ignoreWater,
                    SimulationStage.Final
                );
                break;
            }

            RaycastHit hit;
            if ((Physics.Raycast(ray, out hit, deltaDistance, _layerMask) &&
                    (hit.collider != null &&
                        hit.collider.gameObject != null &&
                        hit.collider.gameObject.GetComponentInParent<Part>() != Part)) // Ignore the part firing the projectile.
                ||
                ...
                switch (stage) {
                    case SimulationStage.Refining | SimulationStage.Normal: // Perform a more accurate final step for the collision.
                    {
                        ...
                        BallisticTrajectorySimulation(
                            ...
                            timeStep / 4f,
                            ignoreWater,
                            timeStep > 5f * Time.fixedDeltaTime ?
                                SimulationStage.Refining :
                                SimulationStage.Final
                        );
                        break;
                    }
                    case SimulationStage.Final: {
                        ...
                        {
                            totalElapsedTime += (hit.point - position).magnitude / velocity.magnitude;
            ...
            totalElapsedTime += timeStep;
            elapsedTime += timeStep;
            if (stage != SimulationStage.Final && drawLines)
                DrawLines(position);
        }
        return totalElapsedTime;
    }

    void DrawLines(Vector3 position) {
        var visualOffset = visualRelativeToVessel ?
            AIUtils.PredictPosition(Vector3.zero, activeVesselVelocity, activeVesselAcceleration, totalElapsedTime) :
            Vector3.zero;
